...
if(empty($_SESSION["username"])){
	...
	if(empty($vat_id) || empty($individual_id) || empty($vat_number) || empty($vat_registered_date) || empty($vat_flat_rate) || empty($flat_rate_first_year) || empty($flat_rate_after_first_year) || empty($vat_return_quarter)) {	
		...
		if($resultVatNumber->num_rows > 0){
			...
			if(isset($_POST['quarters_years_string'])){
				...
				/* Fetch Individual mail id */
				$resultIndividual = mysqli_query($db->db_conn, "SELECT individuals.id as id, individuals.firstname as firstname, individuals.surname as surname,  emails.email as email FROM individuals INNER JOIN emails ON individuals.id  = emails.individual_id AND individuals.id = $individual_id LIMIT 1");
				if($resultIndividual->num_rows > 0){
					while($rowIndividual = $resultIndividual->fetch_array()){
						$toEmail = $rowIndividual['email'];
						$firstname = $rowIndividual['firstname'];
						$fullname = $rowIndividual['firstname']." ".$rowIndividual['surname'];
					}
				}
				...
				foreach($all_quarters_years as $quarter_year){
					...
					$purpose = "Individual VAT Submitted";
					...
					if(mysqli_num_rows($result1) > 0){
						...
						if(isset($_POST['quarters_years_string'])){
							$result2 = mysqli_query($db->db_conn, "INSERT INTO   vats_submission_quarters_years_and_quarters_due(vat_id, individual_id, quarter_year, quarter_due, is_submitted_to_hmrc) values($vat_id, $individual_id, '$quarter_year', $quarter_due, '$is_submitted_to_hmrc')");
						}
					}
					...
					if($is_submitted_to_hmrc == 'Yes'){
						$message = "Dear ".$firstname.", \r\n \r\n \r\n"." VAT is submitted for quarter month : " .date('F Y', strtotime('01-'.$quarter_year)). " . Thank you very much " . $fullname . ".";
						...
						if($previous_mail_send_for_quarter_year == "No"){
							...
							$flagIndividualEmailSend = mail($toEmail, $subject, $message, $headers);

							if($flagIndividualEmailSend){
								//Save Email  into  emails_sent_to_clients
								$emailIndividualSaveResult = mysqli_query($db->db_conn, "INSERT INTO emails_sent_to_clients(individual_id, subject, message, datetime, status, purpose ) VALUES ($individual_id, '$subject', '$message', '$datetime', '1', '$purpose')");
							}else{
								//Save Email  into  emails_sent_to_clients
								$emailIndividualSaveResult = mysqli_query($db->db_conn, "INSERT INTO emails_sent_to_clients(individual_id, subject, message, datetime, status, purpose) VALUES ($individual_id, '$subject', '$message', '$datetime', '0', '$purpose')");
							}
						}
					}
				}
			...
			if ($result) {
				echo json_encode(array('status'=>'success', 'message' => 'Individual VAT has been successfully updated.'));
			} else {
				echo json_encode(array('status'=>'fail', 'message' => 'Individual VAT updation error.'));
			}
		}
	}
